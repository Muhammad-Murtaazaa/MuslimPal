  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.7fr_1fr]">
      <section className="space-y-5">
        <form onSubmit={submit} className="rounded-2xl border border-emerald-900/15 bg-white/70 p-4 backdrop-blur-md shadow-sm">
          <label className="mb-3 block text-xs uppercase tracking-[0.12em] text-emerald-900/70">AI Scholar Query</label>
          <div className="group flex items-center rounded-2xl border border-emerald-900/20 bg-[#FCFCF9] px-3 transition-all focus-within:border-[#0F766E] focus-within:ring-2 focus-within:ring-[#0F766E]/25">
            ...
            <button
              ...
              className="rounded-xl bg-[#064E3B] px-4 py-2 text-sm text-white transition disabled:opacity-60"
            >
              {loading ? 'Thinking...' : cooldownSeconds > 0 ? `Wait ${cooldownSeconds}s` : 'Analyze'}
            </button>
          </div>
          <div className="pointer-events-none mt-2 h-1 w-full rounded bg-gradient-to-r from-transparent via-[#0F766E]/60 to-transparent opacity-0 transition duration-300 group-focus-within:opacity-100" />
        ...
        {result ? (
          ...
          <div className="rounded-2xl border border-dashed border-emerald-900/20 bg-white/60 p-8 text-center text-sm text-emerald-900/70">
            Submit a question to generate scholarly insights and takeaways.
          </div>
        )}
      </section>

      <aside className="space-y-4 rounded-2xl border border-emerald-900/15 bg-white/70 p-4 backdrop-blur-md">
        <h2 className="font-serif text-xl text-emerald-950">Referenced Sources</h2>
        {result?.sources?.length ? (
          result.sources.map((source) => (
            <article key={source.href} className="rounded-xl border border-emerald-900/10 bg-emerald-50/40 p-3">
              <p className="font-medium text-emerald-950">{source.title}</p>
              <a
                href={source.href}
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-[#0F766E] hover:text-[#064E3B]"
              >
          ...
          <p className="rounded-xl border border-dashed border-emerald-900/20 bg-emerald-50/30 p-3 text-xs text-emerald-900/75">
