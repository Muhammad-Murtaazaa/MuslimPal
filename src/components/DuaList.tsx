  ...
  return (
    <section className="space-y-6">
      <header className="rounded-3xl border border-emerald-900/15 bg-white/75 p-5 shadow-sm backdrop-blur-md">
        <h1 className="font-serif text-3xl text-emerald-950">Daily Duas and Zikr</h1>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
          <input
            ...
            className="w-full rounded-xl border border-emerald-900/20 bg-[#FCFCF9] px-3 py-2 text-sm outline-none transition focus:border-[#0F766E] focus:ring-2 focus:ring-[#0F766E]/20"
          />

          <select
            value={category}
            onChange={(event) => setCategory(event.target.value as DuaCategory | 'All')}
            className="rounded-xl border border-emerald-900/20 bg-[#FCFCF9] px-3 py-2 text-sm outline-none transition focus:border-[#0F766E]"
          >
            {categories.map((item) => (
        ...
        <div className="mt-4 rounded-xl border border-emerald-900/15 bg-emerald-50/60 p-3">
          <p className="text-xs font-semibold uppercase tracking-[0.1em] text-emerald-900/70">Daily Zikr Tracker</p>
      ...
      <div className="grid gap-4">
        {filtered.map((dua) => (
          <article
            key={dua.id}
            className="rounded-2xl border border-emerald-900/15 bg-white/80 p-5 shadow-sm backdrop-blur-md"
          >
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="font-semibold text-emerald-950">{dua.title}</h2>
            ...
            <p dir="rtl" className="mt-4 text-right text-lg leading-9 text-emerald-950 md:text-2xl md:leading-[3rem]">
              {dua.arabic}
            </p>

            <div className="mt-4 space-y-2">
              <p className="text-sm italic text-emerald-900/90">{dua.transliteration}</p>
              <p className="text-sm text-emerald-900/80">{dua.translation}</p>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              <button
                ...
                className="rounded-md border border-emerald-900/20 px-2 py-1 font-medium text-emerald-900 hover:bg-emerald-50"
              >
        ...
        {!filtered.length ? (
          <div className="rounded-2xl border border-dashed border-emerald-900/20 bg-white/60 p-6 text-center text-sm text-emerald-900/75">
